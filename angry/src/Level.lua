...
function Level:update(dt)
    ...
    if love.keyboard.wasPressed('space') and
        self.launchMarker.launched and
        not self.launchMarker.triplets and
        not self.launchMarker.collided then
        self.launchMarker:spawnTripleAliens()
    end
    ...
    if self.launchMarker.launched then
        ...
        local topX, topY, lowerX, lowerY = 0, 0, 0, 0
        local topVelX, topVelY, lowerVelX, lowerVelY = 0, 0, 0, 0
        if self.launchMarker.triplets then
            topX, topY = self.launchMarker.topAlien.body:getPosition()
            topVelX, topVelY = self.launchMarker.topAlien.body:getLinearVelocity()

            lowerX, lowerY = self.launchMarker.lowerAlien.body:getPosition()
            lowerVelX, lowerVelY = self.launchMarker.lowerAlien.body:getLinearVelocity()
        end

        -- if we fired our alien to the left or it's almost done rolling, respawn
        if xPos < 0 or topX < 0 or lowerX < 0 or
            ((math.abs(xVel) + math.abs(yVel) < 1.5) and
                (math.abs(topVelX) + math.abs(topVelY) < 1.5) and
                (math.abs(lowerVelX) + math.abs(lowerVelY) < 1.5)) then
            self.launchMarker.alien.body:destroy()
            if self.launchMarker.triplets then
                self.launchMarker.topAlien.body:destroy()
                self.launchMarker.lowerAlien.body:destroy()
            end
