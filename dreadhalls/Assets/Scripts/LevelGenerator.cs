...
public class LevelGenerator : MonoBehaviour {
	...
	public static int curMaze = 0;
	
	public double holesDensity;
	...
	void Start () {
		...
		canvas.GetComponentInChildren<Text>().text = curMaze.ToString();;

		maxHolesNumber = (int)(mazeSize * mazeSize * holesDensity);
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				// create holes
				if (holesNumber < maxHolesNumber && Random.value < holesDensity * 1.1)
				{
					holesNumber++;
				}
				else
